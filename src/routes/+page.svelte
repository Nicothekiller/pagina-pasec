<script lang="ts">
	import { resolve } from '$app/paths';
</script>

<div class="container mx-auto p-8">
	<h1 class="text-5xl font-extrabold text-center text-gray-900 mb-6">
		Bienvenido al E-Portafolio de PASEC
	</h1>
	<p class="text-lg text-center text-gray-700 mb-12 max-w-3xl mx-auto">
		Mi nombre es Nicolás Naranjo, y este portafolio digital es una muestra de mi experiencia
		realizando servicio comunitario y los aprendizajes de la clase de PASEC. Aquí podrás observar mi
		crecimiento personal y profesional, resaltando cómo esta experiencia ha cambiado mi perspectiva
		sobre la ciudadanía responsable.
	</p>

	<section class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
		<div class="bg-white p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300">
			<h2 class="text-2xl font-semibold text-gray-800 mb-3">Producto Mejorado</h2>
			<p class="text-gray-600">
				Una muestra de mi trabajo en clases que muestra un aprendizaje significativo, junto con una
				explicación detallada de su impacto.
			</p>
			<a
				href={resolve('/mejorado')}
				class="text-blue-600 hover:text-blue-800 font-medium mt-4 inline-block"
				>Ver más &rarr;</a
			>
		</div>

		<div class="bg-white p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300">
			<h2 class="text-2xl font-semibold text-gray-800 mb-3">Ciudadanía Responsable</h2>
			<p class="text-gray-600">
				Una reflexión de como mi definición sobre la "ciudadanía responsable" a cambiado a través
				del tiempo.
			</p>
			<a
				href={resolve('/ciudadania')}
				class="text-blue-600 hover:text-blue-800 font-medium mt-4 inline-block"
				>Ver más &rarr;</a
			>
		</div>

		<div class="bg-white p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300">
			<h2 class="text-2xl font-semibold text-gray-800 mb-3">Valores Esenciales</h2>
			<p class="text-gray-600">
				Tres valores esenciales que guían mi acción ciudadana y cómo fueron importantes durante mis
				horas de servicio.
			</p>
			<a
				href={resolve('/valores')}
				class="text-blue-600 hover:text-blue-800 font-medium mt-4 inline-block"
				>Ver más &rarr;</a
			>
		</div>

		<div class="bg-white p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300">
			<h2 class="text-2xl font-semibold text-gray-800 mb-3">Compromisos Personales</h2>
			<p class="text-gray-600">
				Dos compromisos específicos como estudiante y futuro profesional, incluyendo indicadores y
				acciones concretas.
			</p>
			<a
				href={resolve('/compromisos')}
				class="text-blue-600 hover:text-blue-800 font-medium mt-4 inline-block"
				>Ver más &rarr;</a
			>
		</div>

		<div class="bg-white p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300">
			<h2 class="text-2xl font-semibold text-gray-800 mb-3">Video Carta</h2>
			<p class="text-gray-600">
				Un mensaje en video dirigido a mi yo del futuro, reflexionando sobre el impacto de esta
				experiencia.
			</p>
			<a
				href={resolve('/video')}
				class="text-blue-600 hover:text-blue-800 font-medium mt-4 inline-block"
				>Ver más &rarr;</a
			>
		</div>
	</section>
</div>
